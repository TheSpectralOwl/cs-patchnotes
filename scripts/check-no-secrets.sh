...
SECRET_VARS='MEILI_MASTER_KEY|TUNNEL_TOKEN|ANTHROPIC_API_KEY'

# Match an ASSIGNMENT of a secret var to a literal value:
#   <ws>(export )?["']?SECRET["']? <ws> (:|=) <ws> <first value char>
# The var must be the key at line start (so the inner `MEILI_MASTER_KEY:?` of a
# `${MEILI_MASTER_KEY:?err}` interpolation never trips), and the first value
...
if leaks="$(git grep -nE "$PATTERN" -- ':!*.example' ':!.planning/')"; then
