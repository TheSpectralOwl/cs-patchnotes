...
function note({ title, date, game, gid, body }) {
...
test("builds a disposable index from Markdown and applies query filters", () => {
  ...
  fs.writeFileSync(path.join(notesDir, "csgo.md"), note({ title: "CS:GO Update", date: "2021-01-01", game: "csgo", gid: "1", body: "# CS:GO Update\n\nImproved smoke grenades.\n" }));
  fs.writeFileSync(path.join(notesDir, "cs2.md"), note({ title: "CS2 Update", date: "2024-01-01", game: "cs2", gid: "2", body: "# CS2 Update\n\nImproved smoke grenades and maps.\n" }));
